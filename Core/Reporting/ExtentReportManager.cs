...
public static class ExtentReportManager
{
    ...
    public static void Initialize()
    {
        ...
        var reportDir = Environment.GetEnvironmentVariable("PARALLEL_WORKER_REPORT_PATH");
        if (string.IsNullOrWhiteSpace(reportDir))
            reportDir = ConfigReader.GetValue("ReportPath");
        if (string.IsNullOrWhiteSpace(reportDir))
    ...
    private static void InjectReportCustomizations(string reportPath)
    {
        ...
        // Append after spark-script.js so Chart/statusGroup exist; do not replace spark's script slot.
        const string sparkScriptMarker =
            "<script src=\"https://cdn.jsdelivr.net/gh/extent-framework/extent-github-cdn@c439bdbc7bade3d85af3b90e9e69c30ebc87445f/spark/js/spark-script.js\"></script>";
        if (html.Contains(sparkScriptMarker))
            html = html.Replace(sparkScriptMarker, $"{sparkScriptMarker}\n{scriptBlock}");
        else
